...
| `--probe-containers` | 按内容探测改名的容器镜像（无 `.iso` 扩展名也会识别并展开）；默认关闭（仅按扩展名识别），开启后扫描目录时每个 ≥ 1 MiB 且扩展名不匹配的文件多一次打开+读取 / content-probe renamed container images (files without the `.iso` extension are also recognized and expanded); off by default (extension-based only), enabling it costs an extra open+read per file ≥ 1 MiB during directory scans |
