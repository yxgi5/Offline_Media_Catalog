...
- `probe()`：按内容检测文件是否属于该格式（可选：仅 `--probe-containers` 启用时调用）/ content-based detection of whether a file belongs to this format (optional: only called when `--probe-containers` is enabled)
