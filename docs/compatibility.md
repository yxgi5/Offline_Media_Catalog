...
| 残留"假根" FE / leftover "fake root" FE | FSD 后第一个目录 FE 的 extent 极小（88 字节），只有 2 条垃圾 FID / first dir FE after FSD has a tiny extent (88 bytes) with only 2 junk FIDs | 校验每个候选目录的子目录引用（名字无控制字符且 ICB 可解析为有效目录 FE），取第一个通过校验者作根（目录树按 DFS 先序写盘）；全部失败时以条目最多者兜底 / validate each candidate's child directory references (clean names and ICBs resolving to real directory FEs) and pick the first passing one as root (depth-first head tree); fall back to the largest candidate only when none resolve |
